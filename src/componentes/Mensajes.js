...
class Mensajes extends Component {
  constructor(props) {
    ...
    data_form_msj_user.append(
      "propsmensaje_datos_msj_user",
      datos_msj_user.propsmensaje_datos_msj_user
    );
    data_form_msj_user.append(
      "iduserlogin_datos_msj_user",
      datos_msj_user.iduserlogin_datos_msj_user
    );

    axios({
      method: "post",
      url:
        "https://turnmyapp.com/ws_turnmyapp/get/get_users_by_product_msj/validacion",
  ...
  toggle(id_user_chat, conversation_id) {
    ...
    data_form_msj_user_chat.append(
      "propsmensaje_datos_msj_user",
      datos_msj_user_chat.propsdatos_msj_user_chat
    );
    data_form_msj_user_chat.append(
      "iduserlogin_datos_msj_user",
      datos_msj_user_chat.iduser_datos_msj_user_chat
    );

    axios({
      method: "post",
      url:
        "https://turnmyapp.com/ws_turnmyapp/get/get_user_by_product_chat/validacion",
    ...
    setInterval(
      function() {
        ...
        data_form_msj_user_chat.append(
          "propsmensaje_datos_msj_user",
          this.state.propsmensaje
        );
        data_form_msj_user_chat.append(
          "iduserlogin_datos_msj_user",
          this.state.id_user_interesado
        );
      }.bind(this),
      1000
    );
  ...
  onSubmit = e => {
    ...
    if (datos.input_message === "") {
      ...
      axios({
        method: "post",
        url:
          "https://turnmyapp.com/ws_turnmyapp/get/insertar_msj__chat/validacion",
        data: data_form,
        config: { headers: { "Content-Type": "multipart/form-data" } }
      })
                      ...
                      <div className="  ">
                        ...
                        >
                          <img
                            ...
                            style={{ width: "20%;" }}
                            src={`https://turnmyapp.com/files/users/${
                              datos.image
                            }`}
                            alt={datos.name}
                          />
                          <div className="card-body">
                            <h4 className="card-title" key={datos}>
                              {datos.name}
                            </h4>
                    ...
                    <li style={{ width: "100%" }}>
                      <div className="msj macro">
                        ...
                        <div className="text text-l">
                          <p>
                            <span key={datos} className="name_chat_text">
                              {datos.name}
                            </span>:
                          </p>
                          <p>
                            <span key={datos} className="message_chat_text">
                              {datos.message}
                            </span>
                          </p>
                          <p>
                            <small key={datos}>{datos.created_at}</small>
                          </p>
                        </div>
                      </div>
                    </li>
              ...
              <form onSubmit={this.onSubmit}>
                    <div className="msj-rta macro">
                      ...
                      >
                        <input
                          type="text"
                          ref={this.input_message}
                          className="mytext"
                          placeholder="Ingresa mensaje"
                        />
                        <input
                          type="hidden"
                          ref={this.id_user_login}
                          className="mytext"
                          value={this.state.id_user_login}
                        />
                        <input
                          type="hidden"
                          ref={this.id_user_conversation}
                          className="mytext"
                          value={this.state.id_user_conversation}
                        />
                        <input
                          type="hidden"
                          ref={this.id_user_interesado}
                          className="mytext"
                          value={this.state.id_user_interesado}
                        />
                      </div>
                    </div>
                    <div className="div_btn_send_chat">
                      <button
                        type="submit"
                        className="btn btn-  btn_form_turn"
                      >
                        <span className="span_btn_send_chat">
